...
function thrive_display_content_with_ads( $content ) {
	...
	if ( is_singular() ) {
		...
		if ( $_after_paragraphs != false ) {
			...
			$ad_size = explode('x', $_after_paragraphs['size']);

			$ad_content = '<div style="width: '. $ad_size[0] .'px; height: '. $ad_size[1] .'px;"class="' . _thrive_get_in_content_ad_container_class( $_after_paragraphs['parent'] ) . '">' . $_after_paragraphs['embed_code'] . '</div>';
