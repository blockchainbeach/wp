...
ThriveApp.social_icons_hidden = jQuery( 'header .bh' ).attr( 'data-social' ) === 'off' || jQuery( '.fln' ).length === 0;
...
ThriveApp.menu_float = {
	...
	handle: function () {
		...
		if ( this.scroll_dir == 'down' ) {
			...
			if ( ThriveApp.social_icons_hidden && this.current_scroll_top > _nextElement ) {
			...
			if ( ! ThriveApp.social_icons_hidden ) {
				if ( this.current_scroll_top < _bottom_limit ) {
...
jQuery( window ).scroll( function () {
	...
	// SUPP2430 TODO Please check this @Radu. I have elliminated (&& ThriveApp.is_singular) from the conditional, function which i don't quite know what it does, but is the cause of this problem
	if ( ThriveApp.hasFloatingMenu == '1' ) {
